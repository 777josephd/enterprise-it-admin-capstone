...
### Finding 12: Auto-provisioned SharePoint sites created by Microsoft 365 group creation
**Category:** Observed behavior  
**Location:** SharePoint Admin Center > Active Sites  
**Observed:** Two sites were present in Active Sites before any manual SharePoint provisioning: All Company (auto-provisioned at tenant creation) and SterlingIT-Team (auto-provisioned when the SterlingIT-Team Microsoft 365 group was created during the Entra ID week). No SharePoint administrative action was taken to create either site.  
**Implication:** Microsoft 365 group creation triggers automatic SharePoint Team site provisioning as part of unified resource creation. Administrators should account for this behavior when auditing SharePoint site inventories: not all sites in Active Sites were explicitly created by an administrator. Sites connected to Microsoft 365 groups are deleted when the group is deleted.  

---

### Finding 13: Team sites and Communication sites use different administrative role models
**Category:** Observed behavior  
**Location:** SharePoint Admin Center > Active Sites > site membership panels  
**Observed:** Team sites expose Owners and Members as Microsoft 365 group-backed roles with associated email addresses. Communication sites expose Site admins as the primary administrative role without a backing Microsoft 365 group.  
**Implication:** The permission management approach differs by site type. Team site membership is managed through Microsoft 365 group membership, which propagates across all services connected to that group. Communication site access is managed directly at the site level. Administrators should use the appropriate management surface for each site type to avoid permission inconsistencies.  

---

### Finding 14: Custom permission levels available at Business Basic tier 
**Category:** Observed behavior  
**Location:** SharePoint site > Site Settings > Site Permissions > Permission Levels  
**Observed:** The Add a Permission Level button was present and functional on the Sterling IT Support Team site. A custom permission level (Helpdesk Technician) was created successfully with granular list and site permission selections.  
**Implication:** Custom permission level creation is not gated behind a higher tier at Business Basic. Administrators can create role-specific permission levels reflecting the principle of least privilege without requiring additional licensing. Custom levels are site-collection scoped and do not propagate to other sites automatically.  

---

### Finding 15: SharePoint Online enforces a minimum of 100 major versions
**Category:** Observed behavior  
**Location:** SharePoint site > Library Settings > Versioning Settings  
**Observed:** When configuring major version retention in a document library, SharePoint Online rejected values below 100. The interface requires a number between 100 and 50000.  
**Implication:** SharePoint Online enforces a higher minimum version floor than SharePoint Server on-premises, where lower values are permitted. Administrators migrating version retention policies from on-premises environments should account for this platform difference. In practice, 100 versions is a reasonable floor for most organizations.  

---

### Finding 16: Default external sharing configuration is maximally permissive
**Category:** Security observation  
**Location:** SharePoint Admin Center > Policies > Sharing  
**Observed:** The tenant-wide external sharing setting was configured to Anyone (most permissive) for both SharePoint and OneDrive. The default link type was set to Anyone with the link with Edit permission as default. Guest access expiration and verification code reauthentication were both disabled. Domain restrictions and security group restrictions on external sharing were both disabled.  
**Implication:** The out-of-box Microsoft 365 tenant configuration allows any user to generate an anonymous edit link to any document and share it externally without authentication requirement, expiration, or domain restriction. This is a realistic finding in inherited tenant scenarios. Recommended immediate remediations in a production environment: change default link type to Only people in your organization; enable guest access expiration at 60 days maximum; enable verification code reauthentication at 30 days; restrict external sharing to New and existing guests minimum to require authentication for all external access.  
...
### Features confirmed available at Business Basic tier

The following features were confirmed fully available without additional licensing:

| Feature | Location | Notes |
|---|---|---|
| Full user lifecycle management | Entra ID | Create, license, reset, block, restore, delete, recover |
| Security group and Microsoft 365 group management | Entra ID | Both group types fully functional |
| MFA via Security Defaults | Entra ID | Tenant-wide enforcement available |
| Sign-in and audit log access | Entra ID | Available; 7-day retention limit applies |
| Shared mailbox creation and delegation | Exchange Online | Full Access and Send As both configurable |
| Distribution list management | Exchange Online | Full configuration available |
| Mail flow rules | Exchange Online | Full rule creation and enforcement |
| Message trace | Exchange Online | Full trace functionality available |
| SharePoint site provisioning | SharePoint Online | Team and Communication sites both available |
| Custom permission levels | SharePoint Online | Not gated; fully configurable |
| Document library versioning | SharePoint Online | Available; minimum 100 versions enforced |
| OneDrive storage limit configuration | SharePoint Online | Per-user limit configurable by admin |
| External sharing controls | SharePoint Online | Full policy configuration available |

---

*This findings file covers the complete M365 Administration Lab across all three weeks. All findings are final.*
